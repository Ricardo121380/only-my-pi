...
test("session composer owns one read-only runtime, ceiling, private stores and Web child override", async (t) => {
  ...
  assert.match(researcher, /subagentOnlyExtensions: .*safe-web-guard\.mjs, .*fake-web\/index\.ts/u);
  ...
  const guard = await fs.readFile(path.join(runtimeRoot, "safe-web-guard.mjs"), "utf8");
  assert.doesNotMatch(guard, /process\.env\.PI_CODING_AGENT_DIR\s*=/u);
  assert.doesNotMatch(guard, /jiti|import\(/u);
  assert.match(guard, /delete process\.env\.PI_ALLOW_BROWSER_COOKIES/u);
  assert.match(guard, /delete process\.env\.FEYNMAN_ALLOW_BROWSER_COOKIES/u);
...
test("source-review-v2 reserves its exact root token and cost budget by context depth", async () => {
  ...
  assert.deepEqual(Object.fromEntries(plan.nodes.map((node) => [node.id, node.budget.maxTokens])), { explore: 12000, review: 6000, synthesize: 12000, verify: 20000 });
