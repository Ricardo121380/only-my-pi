...
function render(bytes) {
  ...
  return { changed: true, bytes: Buffer.from(`${text}${separator}${BLOCK}`) };
}
...
export class ShellProfileService {
  ...
  async apply(reviewed) {
    ...
    try {
      ...
      return Object.freeze({ ok: true, status: "SHELL_PROFILE_CONFIGURED", mutation: true, profileType: profile.type, sourceDigest: plan.sourceDigest, targetDigest: plan.targetDigest, backupCreated: backup !== null });
    } catch (error) {
