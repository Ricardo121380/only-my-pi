...
export async function buildDistributionArchives({ rootDir, outputRoot, work, seed, seedManifest, receipt }) {
  ...
  const platform = receipt.platform ?? "darwin-arm64";
  const archives = [];
  for (const mode of ["full", "thin"]) {
    ...
    const manifest = { formatVersion: 1, kind: "only-my-pi-native-archive", mode, platform,
...
OMP_NODE="$OMP_TEMP/node-v${manifest.node.version}-${platform}"
...
`, { mode: 0o755 });
    ...
    const filename = `only-my-pi-${receipt.version}-${platform}-${mode}.tar.gz`;
