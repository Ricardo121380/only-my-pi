...
export function assertExactPiSubagentsRpcV1Ping(value) {
  ...
  if (!exactStringSet(data.methods, PI_SUBAGENTS_RPC_V1_METHODS)) {
    wireError("ping reply method set differs from audited pi-subagents@0.45.2", "PI_SUBAGENTS_METHOD_DRIFT", {
      expected: PI_SUBAGENTS_RPC_V1_METHODS,
      actual: data.methods,
    });
  }
  if (!exactShape(data.events, PI_SUBAGENTS_RPC_V1_EVENTS)) {
    wireError("ping reply event map differs from audited pi-subagents@0.45.2", "PI_SUBAGENTS_EVENT_DRIFT", {
      expected: PI_SUBAGENTS_RPC_V1_EVENTS,
      actual: data.events,
    });
  }
  if (!exactShape(data.capabilities, PI_SUBAGENTS_RPC_V1_REQUIRED_CAPABILITIES)) {
    wireError("ping reply capability set differs from audited pi-subagents@0.45.2", "PI_SUBAGENTS_CAPABILITY_DRIFT", {
      expected: PI_SUBAGENTS_RPC_V1_REQUIRED_CAPABILITIES,
  ...
  return immutable({ ...normalized, capabilityHash: digestValue(normalized) });
}

export function createPiSubagentsRpcV1Envelope({ requestId, method, params = {}, source = {} } = {}) {
  assertPiSubagentsRpcV1RequestId(requestId);
  if (!PI_SUBAGENTS_RPC_V1_METHODS.includes(method)) {
