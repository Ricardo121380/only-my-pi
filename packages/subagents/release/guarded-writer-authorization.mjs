...
const DEFAULT_SCHEMA = path.join(DEFAULT_ROOT, "schemas", "subagents-guarded-writer-authorization-v1.schema.json");
const MAX_AUTHORIZATION_BYTES = 128 * 1024;
const MAX_AUTHORIZATION_WINDOW_MS = 24 * 60 * 60 * 1000;

export class GuardedWriterAuthorizationError extends Error {
  constructor(message, code, details = {}) {
    super(`subagents guarded writer authorization: ${message}`);
    this.name = "GuardedWriterAuthorizationError";
    this.code = code;
...
function fail(message, code, details) {
  throw new GuardedWriterAuthorizationError(message, code, details);
}
...
export function guardedWriterAuthorizationDigest(document) {
  return sha256(withoutKey(withoutKey(document, "$schema"), "authorizationDigest"));
}

function validateSemantics(document) {
  if (document.authorizationDigest !== guardedWriterAuthorizationDigest(document)) {
  ...
  const requirement = PROTECTED_EVIDENCE_REQUIREMENTS["guarded-writer-integration"];
  if (!requirement?.writerAuthorized) {
    fail("guarded writer evidence contract is unavailable", "AUTHORIZATION_EVIDENCE_UNSUPPORTED");
  }
  if (document.workspace.automaticIntegration !== false
    || document.workspace.parentDiffVerification !== "required"
    || JSON.stringify(document.writer.allowedPaths) !== JSON.stringify(document.writer.fileClaims)) {
    fail("guarded writer authorization must be handoff-only with exact claims", "AUTHORIZATION_WRITER_SCOPE_INVALID");
  }
}
...
function validateRuntime(document, { matrix, policy, trustPolicy, expectedSourceCommit, now } = {}) {
  ...
  if (signer.publicKeyFingerprint !== document.signer.publicKeyFingerprint
    || !signer.evidenceIds.includes("guarded-writer-integration")) {
...
export function validateGuardedWriterAuthorization(document, {
  allowTemplate = false,
...
export function loadGuardedWriterAuthorization(file, options = {}) {
  ...
  return validateGuardedWriterAuthorization(document, options);
}

export const SUBAGENTS_GUARDED_WRITER_AUTHORIZATION_SCHEMA = DEFAULT_SCHEMA;
