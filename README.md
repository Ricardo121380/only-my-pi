...
only-my-pi (OMP) is a terminal coding agent built on [Pi](https://github.com/earendil-works/pi), installed for your user account. Run `omp` in a project to inspect code, investigate problems, and make project-local changes after explicit approval.
...
> **Published Preview: [v0.3.0-preview.1](https://github.com/Ricardo121380/only-my-pi/releases/tag/v0.3.0-preview.1)**<br>
...
| Manage a user-local installation explicitly | Preview changes before applying them; verify release bundles and diagnose, roll back, or remove the managed stack. |
...
| `MODEL_AUTH_UNAVAILABLE` / `HEADLESS_MODEL_REQUIRED` | Configure authentication through Pi, then select a usable model explicitly. |
