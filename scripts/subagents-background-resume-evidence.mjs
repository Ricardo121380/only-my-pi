...
const HELP = `Usage:
  node scripts/subagents-background-resume-evidence.mjs [--plan] [--authorization-file /absolute/file] [--json]
...
trust policy, an active one-time background-resume authorization, --yes, an
external digest-only signer, and disjoint disposable/config/output roots. The
runner launches two distinct Pi parent processes against one isolated persisted
parent session. It never reads the real ~/.pi or writes verification/protected.
`;
...
export function parseBackgroundResumeEvidenceArgs(argv) {
  const output = {
    operation: "plan",
    yes: false,
    json: false,
...
export async function executeBackgroundResumeEvidence(argv, {
  rootDir,
  sourceInspector = defaultSourceInspector,
  scenarioRunnerFactory = createPiBackgroundResumeScenarioRunner,
  ...
  authorizationLoader = loadBackgroundResumeAuthorization,
  now = () => Date.now(),
  hostEnvironment = process.env,
} = {}) {
  const args = parseBackgroundResumeEvidenceArgs(argv);
  ...
  const capturePlan = createBackgroundResumeCapturePlan({
    authorization,
    matrix,
    policy,
    trustPolicy,
  ...
  if (!plan.runnable || authorization === null) {
    throw Object.assign(new Error(`protected background resume is unavailable: ${plan.blockers.join(", ")}`), {
      code: "PROTECTED_BACKGROUND_RESUME_UNAVAILABLE",
    });
  }
  const roots = validateLiveCaptureRoots({
    repositoryRoot,
  ...
  const capture = await captureProtectedBackgroundResumeEvidence({
    ...
    expectedSourceCommit: source.sourceCommit,
    scenarioRunner,
    signer: signerFactory({ command: args.signerCommand }),
    now: now(),
  });
  ...
  if (!sourceAfter.clean || sourceAfter.sourceCommit !== source.sourceCommit) {
    throw Object.assign(new Error("source changed during protected background resume capture"), {
      code: "BACKGROUND_RESUME_SOURCE_CHANGED",
    });
  }
  ...
  return Object.freeze({
    ...
    capture: {
      ...
      usage: capture.usage,
      evidence: { "background-resume": capture.evidence["background-resume"].evidenceDigest },
    },
    staged,
    exitCode: 0,
  });
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const result = await executeBackgroundResumeEvidence(argv);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  main().then(
    ...
    (error) => {
      process.stderr.write(`subagents-background-resume-evidence: ERROR ${error.code ?? "UNEXPECTED"}: ${error.message}\n`);
      process.exitCode = 1;
    },
  );
}

export { HELP as SUBAGENTS_BACKGROUND_RESUME_HELP, ROOT as REPOSITORY_ROOT };
